# ゲーム制作 ポートフォリオ
* 私が現在までに作ったゲーム作品について列挙しています
  * なお、過去作品に関しては名義が違うものがあります
* 本ページは Markdown で記述し、pandoc で出力しています
...
## Logically
[ゲーム内BGM](https://soundcloud.com/thirsty-three/sets/logically-ost)
...
* 使用言語: Hot Soup Processor 3
* 使用グラフィックAPI: HSP標準命令(GDI)
* 概要: 頂点計算・隠面消去・ラスタライズのライブラリを自力実装し、2D三角形描画命令のみで3D描画を行い作成したシューティングゲーム。
* 工夫点: このゲームは、自力で3Dのラスタライズを実装し、3D描画に関する理解を深めてみようと思って作成した。
  最初はワイヤーフレームで作成し、そのあと単色ポリゴンを描画できるようにした。ポリゴンの順序決定にはZソート法を用いた。(この点で最も苦労したのは、カメラの背面や至近の物体を描画対象から外すアルゴリズムだった。カメラの真横に物体が来たときや、カメラの眼の前に物体が来たときに、1枚の大きなポリゴン(異常なラスタライズがなされている)で画面が覆われてしまうのを改善するのに時間がかかった。)

  ![3D描画デモ](3d_demo.gif)

  また、一通り3D描画の仕組みが整ったのち、そのライブラリ向け3Dモデルを製作するツールも作成した。参考動画にある宇宙船(自キャラ)もこのツールで作成した。

  ![3Dモデル作成ツール](modeltool.png)
